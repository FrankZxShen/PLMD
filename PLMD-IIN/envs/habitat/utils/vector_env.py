...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                elif command == CDDM_PROCESS:
                    local_map = env.CDDM_process_local_map(data)
                    if torch is not None and torch.is_tensor(local_map) and local_map.is_cuda:
                        local_map = local_map.cpu()
                    connection_write_fn(local_map)
    ...
    def CDDM_process_local_map(self, CDDM_inputs):
        ...
        self._is_waiting = True
        for e, write_fn in enumerate(self._connection_write_fns):
            write_fn((CDDM_PROCESS, CDDM_inputs))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        local_map = results
